...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let rerank_request = RerankRequestBuilder::new()
        ...
        .model(RerankModel::RerankEnglishV2)
        .build()?;

    let rerank_response = client.rerank().rerank(&rerank_request).await?;
    println!("Reranked results: {:?}", rerank_response.results);
...
```rust
use voyageai::{VoyageAiClient, VoyageConfig};
use voyageai::search::{SearchBuilder, SearchType};

#[tokio::main]
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    let client = VoyageAiClient::new(VoyageConfig::new("YOUR_API_KEY"));

    let search_request = SearchBuilder::new()
        .query("artificial intelligence")
        .embeddings(vec![/* your document embeddings */])
...
```rust
use voyageai::{VoyageAiClient, VoyageConfig};
use voyageai::search::{SearchClient, SearchType};
...
```rust
use voyageai::{VoyageAiClient, VoyageConfig};
use voyageai::search::{vector_similarity_search, nearest_neighbor_search, nearest_duplicate_search, bm25_search};
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let embeddings = client.embeddings().create_embeddings(&documents).await?;
    ...
    // Nearest Neighbor Search
    let query_embedding = client.embeddings().create_embedding(query).await?;
...
For more detailed information on how to use this SDK, please refer to the [documentation](https://docs.rs/voyageai). Please note that this link may not be active until the crate is published on crates.io. In the meantime, you can find documentation in the `docs` directory of the GitHub repository.
